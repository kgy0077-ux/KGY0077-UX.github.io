...
const WeiminContracts = (() => {
  ...
  async function clearFolder() {
    ...
    try {
      ...
      await new Promise((res, rej) => {
        const tx = db.transaction('handles', 'readwrite');
        tx.objectStore('handles').delete('contractsDir');
        tx.oncomplete = res; tx.onerror = e => rej(e.target.error);
      });
    } catch {}
  }
  ...
  async function add(c) {
    const all = getAll();
    c.id = _uid(); c.createdAt = new Date().toISOString();
    all.unshift(c); await _sync(all); return c;
  }
  async function update(id, updates) {
    ...
    await _sync(all); return all[i];
  }
  async function remove(id) { await _sync(getAll().filter(c => c.id !== id)); }
  ...
  function login(pw) {
    ...
    if (pw === stored) { localStorage.setItem(AUTH_KEY, '1'); return true; }
    return false;
  }
  function logout() { localStorage.removeItem(AUTH_KEY); }
  function changePw(newPw) { localStorage.setItem(PW_KEY, newPw); }
  ...
  return {
    connectFolder, clearFolder, tryRestoreDir, requestDirPermission,
    isConnected: () => !!_dir,
    getAll, add, update, remove, getById, daysUntil,
    getSettings, saveSettings,
    getPendingAlerts, markSent,
    isLoggedIn, login, logout, changePw, requireAuth
  };
})();
